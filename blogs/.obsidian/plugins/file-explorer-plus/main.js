...
function wildcardMatch(pattern, options) {
  ...
  if (
    ...
    throw new TypeError(
      "The second argument must be an options object or a string/boolean separator, but ".concat(
        typeof options,
        " given",
      ),
    )
...
function changeVirtualElementPin(vEl, pin) {
  ...
  if (pin && !vEl.el.hasClass("tree-item-pinned")) {
    ...
    ;(_a = vEl.el.firstChild) == null
      ? void 0
      : _a.insertBefore(pinDiv, vEl.el.firstChild.firstChild)
  } else if (!pin) {
...
function checkPathFilter(filter, file) {
  ...
  if (filter.patternType === "REGEX") {
    ...
    if (
      ...
      re.test(file.basename || file.name)
    ) {
      return true
    }
    ...
    if (
      isMatch2(file.path) ||
      isMatch2(file.path.replace(/.md$/g, "")) ||
      isMatch2(file.basename || file.name)
    ) {
      return true
    }
  } else {
...
var InputFilterNameModal = class extends import_obsidian3.FuzzySuggestModal {
  ...
  getItems() {
    ...
    if (this.actionType === "PIN") {
      filters = filters.concat(
        ((_a = this.plugin.settings) == null ? void 0 : _a.pinFilters.tags) || [],
      )
      filters = filters.concat(
        ((_b = this.plugin.settings) == null ? void 0 : _b.pinFilters.paths) || [],
      )
    } else if (this.actionType === "HIDE") {
      filters = filters.concat(
        ((_c = this.plugin.settings) == null ? void 0 : _c.hideFilters.tags) || [],
      )
      filters = filters.concat(
        ((_d = this.plugin.settings) == null ? void 0 : _d.hideFilters.paths) || [],
      )
    }
  ...
  onChooseItem(chosenFilter) {
    ...
    if (this.actionType === "PIN") {
      ...
      this.plugin.settings.pinFilters.paths = this.plugin.settings.pinFilters.paths.map(
        (filter) => {
          if (filter.name === chosenFilter.name) {
            filter.active = !filter.active
          }
      ...
      this.plugin.settings.hideFilters.tags = this.plugin.settings.hideFilters.tags.map(
        (filter) => {
          if (filter.name === chosenFilter.name) {
            filter.active = !filter.active
          }
      ...
      this.plugin.settings.hideFilters.paths = this.plugin.settings.hideFilters.paths.map(
        (filter) => {
          if (filter.name === chosenFilter.name) {
            filter.active = !filter.active
          }
    ...
    if (this.specificFilter) {
      ...
      pathsActivated =
        this.actionType === "HIDE"
          ? this.plugin.getPathsToHide(files)
          : this.plugin.getPathsToPin(files)
    }
...
var FileExplorerPlusSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian4.Setting(this.containerEl)
  ...
  cleanSettings() {
    this.plugin.settings.hideFilters.tags = this.plugin.settings.hideFilters.tags.filter(
      (filter, index, arr) => {
    ...
    this.plugin.settings.hideFilters.paths = this.plugin.settings.hideFilters.paths.filter(
      (filter, index, arr) => {
        if (index == arr.length - 1) {
          return true
        }
...
function addOnTagChange(plugin) {
  plugin.registerEvent(
    plugin.app.metadataCache.on("changed", (path, data, cache) => {
      ...
      const shouldBePinned = plugin.settings.pinFilters.tags.some((filter) =>
        checkTagFilter(filter, path),
      )
      const shouldBeHidden = plugin.settings.hideFilters.tags.some((filter) =>
        checkTagFilter(filter, path),
      )
        ...
        menu
          ...
          .addItem((item) => {
            const index = plugin.settings.pinFilters.paths.findIndex(
              (filter) =>
                filter.patternType === "STRICT" &&
                filter.type === "FILES" &&
                filter.pattern === path.path,
            )
...
var FileExplorerPlusPlugin = class extends import_obsidian6.Plugin {
  ...
  getPathsToPin(paths) {
    return paths.filter((path) => {
      ...
      const pathFilterActivated = this.settings.pinFilters.paths.some((filter) =>
        checkPathFilter(filter, path),
      )
      if (pathFilterActivated) {
        return true
      }
      const tagFilterActivated = this.settings.pinFilters.tags.some((filter) =>
        checkTagFilter(filter, path),
      )
  ...
  getPathsToHide(paths) {
    return paths.filter((path) => {
      ...
      const pathFilterActivated = this.settings.hideFilters.paths.some((filter) =>
        checkPathFilter(filter, path),
      )
      if (pathFilterActivated) {
        return true
      }
      const tagFilterActivated = this.settings.hideFilters.tags.some((filter) =>
        checkTagFilter(filter, path),
      )
      if (tagFilterActivated) {
        return true
