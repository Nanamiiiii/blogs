...
function getOrderedMatches({ content, regex, cursorIdx, filterMatch, direction }) {
  ...
  return [...previousMatches.reverse(), ...nextMatches.reverse(), ...currentMatches.reverse()]
}
/**
...
const mappingCommands = ["map", "nmap", "noremap", "iunmap", "nunmap", "vunmap"]
function sleep(ms) {
...
class VimrcPlugin extends obsidian.Plugin {
  constructor() {
    ...
    this.onKeydown = (ev) => {
      if (this.settings.fixedNormalModeLayout) {
        ...
        if (!this.isInsertMode && !ev.shiftKey && ev.code in keyMap && ev.key != keyMap[ev.code]) {
  ...
  async updateSelectionEvent() {
    ...
    if (!cm) return
    if (this.getCursorActivityHandlers(cm).some((e) => e.name === "updateSelection")) return
    ...
    vimObject.defineEx("sendkeys", "", async (cm, params) => {
      ...
      for (const key of params.args) {
        if (key.startsWith("wait")) {
          ...
          if (allGood) {
            for (keyEvent of events) window.postMessage(JSON.parse(JSON.stringify(keyEvent)), "*")
  ...
  executeObsidianCommand(commandName) {
    ...
    else throw new Error(`Command ${commandName} doesn't have an Obsidian callback`)
  }
  ...
  defineJsCommand(vimObject) {
    vimObject.defineEx("jscommand", "", (cm, params) => {
      ...
      let currentSelections = this.currentSelection
      var chosenSelection =
        currentSelections && currentSelections.length > 0 ? currentSelections[0] : null
  ...
  defineJsFile(vimObject) {
    vimObject.defineEx("jsfile", "", async (cm, params) => {
      ...
      if (params?.args?.length < 1) throw new Error("Expected format: fileName {extraCode}")
      let extraCode = ""
  ...
  defineSource(vimObject) {
    vimObject.defineEx("source", "", async (cm, params) => {
      if (params?.args?.length > 1) throw new Error("Expected format: source [fileName]")
...
class SettingsTab extends obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new obsidian.Setting(containerEl)
      ...
      .addToggle((toggle) => {
        toggle.setValue(
          this.plugin.settings.fixedNormalModeLayout || DEFAULT_SETTINGS.fixedNormalModeLayout,
        )
    ...
    new obsidian.Setting(containerEl)
      ...
      .addToggle((toggle) => {
        toggle.setValue(
          this.plugin.settings.supportJsCommands ?? DEFAULT_SETTINGS.supportJsCommands,
        )
        toggle.onChange((value) => {
